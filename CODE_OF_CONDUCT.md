# 📜 Code of Conduct

## 🌟 Our Commitment

We as contributors and maintainers of **Schedula** pledge to make participation in our community a respectful, inclusive, and harassment-free experience for everyone.

We are committed to creating an open-source environment where everyone feels welcome regardless of:
- Experience level
- Gender
- Age
- Nationality
- Personal background
- Identity
- Appearance
- Religion
- Technical expertise

---

# 🤝 Expected Behavior

All contributors are expected to:

- Be respectful and supportive
- Use welcoming and inclusive language
- Accept constructive feedback gracefully
- Help beginners and new contributors
- Focus on collaboration and learning
- Maintain professionalism in discussions
- Respect differing opinions and experiences

---

# 🚫 Unacceptable Behavior

The following behaviors are not tolerated:

- Harassment or discrimination of any kind
- Personal attacks or insulting comments
- Hate speech or offensive language
- Trolling or deliberately disruptive behavior
- Sharing private information without consent
- Spam or self-promotion unrelated to the project
- Plagiarism or intentionally harmful contributions

---

# ⚠️ AI Usage Policy

AI tools are allowed for:
- Learning concepts
- Understanding code
- Debugging assistance
- Documentation help
- Improving productivity

However:

❌ Blindly copy-pasting large amounts of AI-generated code without understanding it is discouraged.

Contributors should:
- Understand the code they submit
- Be able to explain their implementation
- Ensure code quality and maintainability
- Follow project architecture and standards

Maintainers reserve the right to reject low-quality or irresponsible AI-generated contributions.

---

# 💬 Communication Guidelines

Please keep all interactions:
- Friendly
- Professional
- Constructive
- Inclusive

When reviewing pull requests:
- Give actionable feedback
- Avoid harsh criticism
- Encourage learning and improvement

---

# 🛠 Reporting Issues

If you experience or witness unacceptable behavior, please contact the maintainers:

Rohit Bhalekar (PA)
- [GitHub](https://github.com/DevROHIT11)
- [LinkedIn](https://www.linkedin.com/in/rohit-bhalekar-1603b02ab)
- Email: rohitwork112004@gmail.com

---

Sandesh Shinde (Maintainer & Collaborator)
- [GitHub](https://github.com/Molza01)
- [LinkedIn](https://www.linkedin.com/in/sandesh-shinde-0255592a2/)
- Email: sandeshshinde5002@gmail.com


---

Tushar Kale (Maintainer & Collaborator)
- [GitHub](https://github.com/TusharKale10)
- [LinkedIn](https://linkedin.com/in/tushar-kale-2a76862aa)
- Email: tusharkale2037@gmail.com 

---

All reports will be reviewed confidentially.

You may:
- Open a private discussion
- Reach out through GitHub
- Contact maintainers respectfully

All reports will be reviewed fairly and handled confidentially.

---

# ⚖️ Enforcement

Project maintainers are responsible for enforcing this Code of Conduct.

They may:
- Remove inappropriate content
- Reject harmful pull requests
- Restrict participation for repeated violations

Actions will be taken based on the severity and frequency of violations.

---

# ❤️ Open Source Spirit

Schedula is built to encourage:
- Learning
- Collaboration
- Innovation
- Community growth

We believe open source should remain welcoming and accessible to everyone.

Thank you for helping make this community positive and supportive 🚀